...
def do_train(
    ...
    for iteration, (images, targets) in enumerate(train_loader, start_iter):
        ...
        if iteration % cfg.VALIDATION.VERBOSE == 0 or iteration == max_iter:
            ...
            labels_query = val_loader.dataset.label_list
            labels_query = np.array([int(k) for k in labels_query])
            feats_query = feat_extractor(model, val_loader, logger=logger)

            labels_gal = GAL_loader.dataset.label_list
            labels_gal = np.array([int(k) for k in labels_gal])
            feats_gal = feat_extractor(model, GAL_loader, logger=logger)

            ret_metric = RetMetric(feats=[feats_gal , feats_query ], labels=[labels_gal , labels_query ])
