...
def build_data(cfg, is_train=True , is_query = True):
    ...
    if is_train:
        ...
        dataset = BaseDataSet(cfg.DATA.GAL_IMG_SOURCE, transforms=transforms, mode=cfg.INPUT.MODE)
        data_loader = DataLoader(dataset,
                                 collate_fn=collate_fn,
                                 shuffle=False,
                                 batch_size=cfg.DATA.TEST_BATCHSIZE,
                                 num_worker=cfg.DATA.NUM_WORKERS
                                 )
    return data_loader
